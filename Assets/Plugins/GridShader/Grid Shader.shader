Shader "Ogxd/Grid"
{
	Properties
	{
		...
		_DeltaX("X Shift", Float) = 0.5
		_DeltaY("Y Shift", Float) = 0.5
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				//// *Custom UV coord delta!
				//o.uv.x = v.uv.x - _DeltaX;
				//o.uv.y = v.uv.y - _DeltaY;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				if (pos.x == 1 || pos.y == 1) {
					...
					if (pos.x == 1 || pos.y == 1) {
						col = _SecondaryColor; //* (1 - fade) + _Color * fade;
						col.a = (1 - fade);
					} else {
